...
c?null:c)||_setGlobalSettings,a)},resetLocalStorage=function(a){chrome.storage.sync.remove(_getGlobalSettings,a)},getCookie=function(a){a+="=";for(var c=document.cookie.split(";"),b=c.length,d=0;d<b;d++){for(var e=c[d];" "==e.charAt(0);)e=e.substring(1);if(0==e.indexOf(a))return e.substring(a.length,e.length)}return""},_isRunningOpera=/Opera|OPR\//.test(navigator.userAgent),_manifestData=chrome.runtime.getManifest(),_globalConfig={childList:!0},_altConfig={attributes:!0,childList:!0,subtree:!0},_body=
document.querySelector("body"),_app=document.querySelector("#app"),_userID=getCookie("i"),_userName=getCookie("p"),_defaultFilter={artists:[],tracks:[]},_defaultMenus={hideFooterMenu:"off",hideHeaderMenu:"off"},_authorQuery=".soundTitle__username, .chartTrack__username a, .playableTile__usernameHeading, .trackItem__username",_trackQuery=".soundTitle__title, .chartTrack__title a, .playableTile__artworkLink, .trackItem__trackTitle",_tagQuery=".soundTitle__tagContent",skipPrevious=!1,oldLocation=location.href,
debugMode=null;getLocalStorage(function(a){"on"===a.debug&&(debugMode=!0,console.log("-=- YellowCloud developer -=-"))},["debug"]);
...
for(var b=document.querySelectorAll(".filter-container"),d=b.length,e=0;e<d;e++)if(!b[a.i].hasChildNodes()){var f=document.createElement("span");f.innerHTML="No "+c+" has been filtered!";b[e].appendChild(f)}},{filter:b})},["filter"])}}(b))}})};k(a,"artists");k(c,"tracks");k(b,"playlists")},webstoreInit=function(){if(_isRunningOpera){var a=document.querySelector("#webstore-link");a.innerText="Opera addons site";a.href="#";a.target=""}},readyStateCheck=function(){setAttributes();settingsSetup();injectedJavascript()};
document.addEventListener("readystatechange",readyStateCheck);setInterval(function(){location.href!=oldLocation&&(debugMode&&console.log("-=- NEW PAGE LOADED -=-"),oldLocation=location.href,readyStateCheck())},100);
...
10);setTimeout(function(){a.style.overflow=null},400);fetchFile(E,"/assets/html/settings.html",settingsInit);fetchFile(H,"/assets/html/filter.html",filterInit);fetchFile(G,"/assets/html/changelog.html");fetchFile(F,"/assets/html/about.html",webstoreInit);for(var I=document.querySelectorAll(".tab"),J=I.length,c={i:0};c.i<J;c={i:c.i},c.i++)I[c.i].addEventListener("click",function(a){return function(){for(var b=0;b<J;b++)I[b].className="tab g-tabs-link";I[a.i].className="tab g-tabs-link active";for(var b=
